...
from typing import Any, Callable, Iterator, Set, Tuple, Union
...
class RedisClient:
    ...
    async def __init_redis(self):
        ...
        while self.conn_alive_flag:
            logger.info(f"Connect Redis<{self.user}:{self.password}@{self.host}:{self.port}>, _RedisClient__conn_success_event={self.__conn_success_event}")
            try:
                ...
                logger.warning(f"Failed to connect Redis<{self.user}:{self.password}@{self.host}:{self.port}>: {exc.__class__.__name__}:{exc}")
    ...
    async def __start(self):
        ...
        while self.conn_alive_flag:
            ...
            if isinstance(self.__pool_client, aioredis.Redis):
                logger.warning(f"Redis<{self.user}:{self.password}@{self.host}:{self.port}> disconnected ... reconnect={self.conn_alive_flag}")
...
class RedisSentinelClient:
    ...
    async def __init_redis(self):
        ...
        while self.conn_alive_flag:
            logger.info(f"Connect Redis<{self.user}:{self.password}@{self.sentinels}>, _RedisSentinelClient__conn_success_event={self.__conn_success_event}")
            try:
                ...
                logger.warning(f"Failed to connect Redis<{self.user}:{self.password}@{self.sentinels}>: {exc.__class__.__name__}:{exc}")
                await asyncio.sleep(self.retry_interval)
    ...
    async def __start(self):
        ...
        while self.conn_alive_flag:
            ...
            if isinstance(self.__sentinel_manager, Sentinel):
                logger.warning(f"Redis<{self.user}:{self.password}@{self.sentinels}> disconnected ... reconnect={self.conn_alive_flag}")
